...
describe('Streamie', () => {
  ...
  //   const initialStreamie = streamie((input: number) => input * 2, { batchSize: undefined });
  ...
  test('map function with batchSize > 1', async () => {
    let mappedStreamieWasDrained = false;
    const initialStreamie = streamie<number, number[], { batchSize: 2 }>((inputs: number[]) => inputs.map(input => input * 2), { batchSize: 2 });
